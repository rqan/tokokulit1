@extends('admin.layout.main')

@section('title', 'Manajemen Ulasan - Panel Admin')

@section('content')
<div class="mb-8">
    <div class="border-minimal border-lightBorder dark:border-darkBorder rounded overflow-x-auto">
        <table class="w-full text-left text-sm">
            <thead class="border-b-minimal border-lightBorder dark:border-darkBorder bg-black/5 dark:bg-white/5 uppercase tracking-widest text-[10px] font-bold">
                <tr>
                    <th class="p-4">Customer</th>
                    <th class="p-4">Order/Invoice</th>
                    <th class="p-4">Rating</th>
                    <th class="p-4">Ulasan</th>
                    <th class="p-4">Tanggal</th>
                    @if(session('role') === 'superadmin' || optional(Auth::user())->role === 'superadmin')
                    <th class="p-4">Aksi</th>
                    @endif
                </tr>
            </thead>
            <tbody class="divide-y divide-lightBorder dark:divide-darkBorder border-t border-lightBorder dark:border-darkBorder">
                @forelse($reviews as $review)
                    <tr class="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                        <td class="p-4">
                            <div class="font-semibold">{{ $review->user->name ?? 'Unknown' }}</div>
                        </td>
                        <td class="p-4">
                            <a href="{{ url('/admin/orders/' . $review->order_id) }}" class="font-mono hover:underline text-lightMain dark:text-darkMain">
                                {{ $review->order->invoice_number ?? 'Pesanan #' . $review->order_id }}
                            </a>
                        </td>
                        <td class="p-4">
                            <div class="flex text-amber-500">
                                @for($i = 1; $i <= 5; $i++)
                                    @if($i <= $review->rating)
                                        <svg class="w-4 h-4 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                    @else
                                        <svg class="w-4 h-4 text-gray-300 dark:text-gray-600 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                    @endif
                                @endfor
                            </div>
                        </td>
                        <td class="p-4 max-w-xs">
                            <p class="truncate" title="{{ $review->review }}">{{ $review->review }}</p>
                        </td>
                        <td class="p-4 text-xs text-lightMuted dark:text-darkMuted">{{ $review->created_at->format('d M Y') }}</td>
                        
                        @if(session('role') === 'superadmin' || optional(Auth::user())->role === 'superadmin')
                        <td class="p-4">
                            <form action="{{ url('/admin/reviews/delete/' . $review->id) }}" method="GET" onsubmit="return confirm('Hapus ulasan ini?');">
                                <button type="submit" class="text-xs uppercase tracking-widest font-bold text-red-500 hover:underline">Hapus</button>
                            </form>
                        </td>
                        @endif
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="p-8 text-center text-lightMuted dark:text-darkMuted">Belum ada ulasan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    </div>
    
    @if(method_exists($reviews, 'links'))
    <div class="mt-6">
        
    </div>
    @endif
</div>
@endsection
